function template(item){
    return `<span class="mention" onclick="eval('window.bus.$emit(\\'mention-click\\', \\'${JSON.stringify(item.original).replace(/\"/gmi, "\\\\\\'")}\\')')">@${item.original.path[0] || item.original.path}</span>`;
}
