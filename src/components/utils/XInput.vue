...
function isRangeAfterNode(range, node) {
  ...
  if (range.compareBoundaryPoints) {
    ...
    if (lastTextNode) nodeRange.selectNodeContents(lastTextNode);
...
export default {
              ...
              {},
              {
                ...
                lookup: function(entry, value) {
                  return (
                    entry.name +
                    "," +
                    entry.path.filter(i => i == 0 || !!i).join(",")
                  );
                },
                menuContainer: false
              },
  ...
  computed: {
    ...
    isTransparent() {
      ...
      else if (typeof this.transparent == "string")
        return this.transparent.toLowerCase() != "false";
    },
    valueModel: {
      get() {
        if(this.type == 'mention'){
      ...
      if (event.keyCode == 8 || event.keyCode == 46) {
        ...
        if (range) {
          ...
          if (node && node.nodeType != 3) {
            // se não for um texto, se for uma tag
            while (node) {
